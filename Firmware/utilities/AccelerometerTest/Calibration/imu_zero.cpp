...
void setup()
{
  ...
  if (range == MPU6050_ACCEL_FS_2)
    gravity_target = 16384;
  else if (range == MPU6050_ACCEL_FS_4)
    gravity_target = 8192;
  else if (range == MPU6050_ACCEL_FS_8)
    gravity_target = 4096;
  else if (range == MPU6050_ACCEL_FS_16)
    gravity_target = 2048;
...
void PullBracketsOut()
{
  ...
  for (int attempt = 1; attempt <= 10; attempt++)
  {
    ...
    if (Done)
      break; // If we found the brackets, exit early
